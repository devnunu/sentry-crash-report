...
export default function WeeklyiOSReportPage() {
  ...
  // 플랫폼 필터 (히스토리)
  const [historyPlatform] = useState<'all' | 'android' | 'ios'>('ios')
  ...
  useEffect(() => {
    const loadTop = async () => {
      setTopLoading(true)
      try {
        const aRes = await fetch('/api/reports/weekly/top?platform=ios').then(r=>r.json())
  ...
  return (
    <div className="container">
      <Group justify="space-between" align="flex-start" mb="sm">
        <div>
          <Title order={2}>🍎 iOS 주간 리포트</Title>
          <Text c="dimmed" size="sm">iOS 플랫폼의 Sentry 주간 크래시 리포트를 생성하고 관리합니다.</Text>
        </div>
      </Group>
