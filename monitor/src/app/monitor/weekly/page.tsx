...
import { Button, Card, Checkbox, Chip, Group, Modal, ScrollArea, Select, SegmentedControl, Stack, Table, Text, TextInput, Title, useMantineTheme } from '@mantine/core'
...
export default function WeeklyReportPage() {
  ...
  const handleSettingsUpdate = async () => {
    ...
    try {
      ...
      notifications.show({ color: 'green', message: '주간 설정 저장 완료' })
      ...
      notifications.show({ color: 'red', message: `주간 설정 저장 실패: ${m}` })
      // 에러 메시지는 5초 후 삭제
      setTimeout(() => {
        setSettingsMessage('')
      }, 5000)
    ...
    const loadTop = async () => {
      ...
      try {
        const [aRes, iRes] = await Promise.all([
          fetch('/api/reports/weekly/top?platform=android').then(r=>r.json()),
          fetch('/api/reports/weekly/top?platform=ios').then(r=>r.json())
        ])
  ...
  return (
    <div className="container">
      <Group justify="space-between" align="flex-start" mb="sm">
        <div>
          <Title order={2}>📈 주간 리포트</Title>
          <Text c="dimmed" size="sm">Sentry 주간 크래시 리포트를 생성하고 관리합니다. 자동 스케줄 설정에 따라 실행되며, 수동 생성도 가능합니다.</Text>
        </div>
      </Group>
      ...
      <Card withBorder radius="lg" p="lg" mt="md">
        ...
        <Stack gap="md">
          ...
          <Group align="center" gap="sm">
            <Button onClick={handleSettingsUpdate} loading={settingsLoading} disabled={(autoEnabled && scheduleDays.length === 0) || !validateTimeFormat(scheduleTime)} variant="light">설정 저장</Button>
            {settingsMessage && (
      ...
      <Card withBorder radius="lg" p="lg" mt="md">
        <Title order={4} mb="sm">⏱️ 스케줄 상태</Title>
        <Text c="dimmed">
          {cronLoading ? '스케줄 상태 불러오는 중…' : (
            cronStatus ? (
              `현재 시간(KST): ${cronStatus.currentTime?.time} (${String(cronStatus.currentTime?.day).toUpperCase()}) · 오늘 실행: ${cronStatus.weeklyReport?.shouldRunToday ? '예' : '아니오'} · 시간 일치: ${cronStatus.weeklyReport?.timeMatch ? '예' : '아니오'} · 설정: ${cronStatus.weeklyReport?.scheduleTime}`
                    ...
                    {reports.map((report) => {
                      return (
                        <Table.Tr key={report.id}>
                          <Table.Td>{`${toKstDate(report.start_date)} ~ ${toKstDate(report.end_date)}`}</Table.Td>
          ...
          <div className="mobile-cards" style={{ marginTop: 16 }}>
            {reports.map((report) => (
              <Card key={report.id} withBorder radius="md" p="md" style={{ marginBottom: 12 }}>
                ...
                <Stack gap={6}>
                  <Text size="xs" c="dimmed">분석 기간</Text>
                  <Text size="sm">{`${toKstDate(report.start_date)} ~ ${toKstDate(report.end_date)}`}</Text>
