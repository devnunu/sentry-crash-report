...
export async function GET(request: NextRequest) {
  try {
    ...
    // 완료된 테스트 모니터 세션 조회
    const allSessions = await db.getMonitorSessions()

    // 테스트 모드이고 완료된 것만 필터링
    const completedTestSessions = allSessions
      .filter(s =>
        s.is_test_mode &&
        s.custom_interval_minutes &&
        (s.status === 'stopped' || s.status === 'expired')
      )
      .slice(0, limit)

    // 각 세션의 히스토리 정보를 별도로 조회
    const historyMonitors = await Promise.all(
      completedTestSessions.map(async (session) => {
        // 히스토리 조회
    ...
    return NextResponse.json(createApiResponse(historyMonitors))
  } catch (error) {
    console.error('Failed to fetch test monitor history:', error)
