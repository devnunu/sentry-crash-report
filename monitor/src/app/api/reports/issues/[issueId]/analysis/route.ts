...
export async function GET(request: NextRequest, { params }: { params: { issueId: string } }) {
  try {
    ...
    const existing = await reportsDb.getIssueAnalysis(platform, params.issueId, reportType, dateKey)
...
export async function POST(request: NextRequest, { params }: { params: { issueId: string } }) {
  try {
    ...
    if (!force) {
      const cached = await reportsDb.getIssueAnalysis(platform, params.issueId, reportType, dateKey)
    ...
    const eventId = await getLatestEventIdForIssue(params.issueId)
    if (!eventId) {
    ...
    const digest = hashPrompt(JSON.stringify({ platform, reportType, dateKey, issue: params.issueId, eventId: details.id }))
    const saved = await reportsDb.upsertIssueAnalysis(platform, params.issueId, reportType, dateKey, analysis, digest)
