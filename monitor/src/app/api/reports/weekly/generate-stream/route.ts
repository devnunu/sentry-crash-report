...
async function generateReportWithLogs(
  ...
  try {
    const { targetWeek, startDate, endDate, sendSlack, includeAI, isTestMode, platform, triggerType } = options

    // 시작 로그
    await writer.write({
      type: 'log',
      message: `🚀 주간 리포트 생성을 시작합니다...`,
    ...
    if (targetWeek) {
      ...
      try {
        ...
        await writer.write({
          type: 'log',
          message: `📅 대상 기간: ${startDate} ~ ${endDate}`,
          timestamp: new Date().toISOString()
        })
      } catch (err) {
        await writer.write({
    ...
    for (let i = 0; i < platforms.length; i++) {
      ...
      await writer.write({
        type: 'log',
        message: `${platformEmoji} ${p.toUpperCase()} 주간 리포트 생성 중... (${i + 1}/${platforms.length})`,
        timestamp: new Date().toISOString()
      })
      ...
      try {
        ...
        await writer.write({
          type: 'log',
          message: `✅ ${platformEmoji} ${p.toUpperCase()} 주간 리포트 생성 완료 (ID: ${result.executionId})`,
          timestamp: new Date().toISOString()
        })
        ...
        await writer.write({
          type: 'error',
          message: `❌ ${platformEmoji} ${p.toUpperCase()} 주간 리포트 생성 실패: ${platformError instanceof Error ? platformError.message : String(platformError)}`,
          timestamp: new Date().toISOString()
        })
      }
    }
    ...
    await writer.write({
      type: 'error',
      message: `❌ 주간 리포트 생성 중 오류가 발생했습니다: ${error instanceof Error ? error.message : String(error)}`,
      timestamp: new Date().toISOString()
    })
  } finally {
    await writer.close()
